...
[ -f $LMP_DIR/setup.done ] && echo "skip lammps setup" || {
    ...
    for TARGET in `ls -d $WORK_DIR/lammps-data-new/*`; do
        omb combo \
            add_files DATA_FILE "$WORK_DIR/lammps-data/$TARGET/*" --abs -\
            add_file_set DP_MODELS "$DP_DIR/model-*/compress.pb" --abs - \
            add_var TEMP 50 100 200 400 600 800 1000 - \
            add_var STEPS 60000 - \
            add_var RES_FORCE 10 - \
            add_var DIST1 $TARGET - \
            add_var DIST2 $TARGET - \
            add_randint SEED -n 10000 -a 0 -b 99999 - \
            set_broadcast SEED - \
            make_files $LMP_DIR/job-{DIST1}-{DIST2}-{TEMP}K-{i:03d}/lammps.in --template $CONFIG_DIR/lammps/lammps.in - \
            make_files $LMP_DIR/job-{DIST1}-{DIST2}-{TEMP}K-{i:03d}/plumed.in --template $CONFIG_DIR/lammps/plumed.in - \
            make_files $LMP_DIR/job-{DIST1}-{DIST2}-{TEMP}K-{i:03d}/run.sh    --template $CONFIG_DIR/lammps/run.sh --mode 755 - \
            done
    done
